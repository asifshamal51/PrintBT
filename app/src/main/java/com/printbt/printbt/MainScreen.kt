...
fun MainScreen(
    ...
    onAddPrinterClick: (BluetoothDevice) -> Unit,
    ...
    Column(
        ...
        // Add Printer Button (visible only in add printer mode)
        if (uiState.connectedDevice != null) {
            Button(
                onClick = { onAddPrinterClick(uiState.connectedDevice!!) },
                modifier = Modifier.fillMaxWidth()
            ) {
                Text("Add Printer")
            }
        ...
        // Print Size Selection (not shown in add printer mode)
        if (!isAddPrinterMode) {
            Row(
                modifier = Modifier.fillMaxWidth(),
                horizontalArrangement = Arrangement.SpaceBetween
